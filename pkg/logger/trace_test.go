...
// TestFromContext_TraceIDBeforeMsg verifies that when a trace ID is present,
// the trace_id field appears before the msg field in the JSON output.
func TestFromContext_TraceIDBeforeMsg(t *testing.T) {
	var buf zapBufferWriteSyncer
	core := zapcore.NewCore(
		getEncoder(),
		&buf,
	...
	ctx := WithTraceID(context.Background(), "abcd1234")
	l := FromContext(ctx)
	l.Info("hello world")

	output := strings.TrimSpace(buf.String())

	traceIdx := strings.Index(output, `"trace_id"`)
	msgIdx := strings.Index(output, `"msg"`)

	if traceIdx == -1 {
		t.Fatalf("trace_id not found in output: %s", output)
	}
	if msgIdx == -1 {
		t.Fatalf("msg not found in output: %s", output)
	}
	if traceIdx > msgIdx {
		t.Fatalf("expected trace_id before msg, but got trace_id at %d and msg at %d in: %s", traceIdx, msgIdx, output)
	}
}
