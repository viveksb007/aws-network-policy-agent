...
import (
	...
	mapsv2 "github.com/aws/aws-ebpf-sdk-go/pkg/mapsv2"
...
func NewBpfClient(policyEndpointeBPFContext *sync.Map, nodeIP string, enablePolicyEventLogs, enableCloudWatchLogs bool,
	...
	ebpfClient := &bpfClient{
		...
		ingressInMemoryMap:        make(map[string]*mapsv2.InMemoryBpfMap),
		egressInMemoryMap:         make(map[string]*mapsv2.InMemoryBpfMap),
	}
	...
	eventBufferFD := 0
	isConntrackMapPresent, isPolicyEventsMapPresent, eventBufferFD, interfaceNametoIngressPinPath, interfaceNametoEgressPinPath, err = ebpfClient.recoverBPFState(ebpfClient.bpfTCClient, ebpfClient.bpfSDKClient, policyEndpointeBPFContext,
...
type bpfClient struct {
	...
	ingressInMemoryMap map[string]*mapsv2.InMemoryBpfMap
	// This is in-memory map backed by egressBpfMap
	egressInMemoryMap map[string]*mapsv2.InMemoryBpfMap
}
		...
		for pinPath, bpfEntry := range bpfState {
			...
			if direction == "ingress" && !updateIngressProbe {
				...
				if found {
					...
					l.ingressInMemoryMap[podIdentifier], err = mapsv2.NewInMemoryBpfMap(&val)
					if err != nil {
				...
				if found {
					...
					l.egressInMemoryMap[podIdentifier], err = mapsv2.NewInMemoryBpfMap(&val)
					if err != nil {
	...
	if ok {
		...
		if ingressProgInfo.Program.ProgFD != 0 {
			...
			if _, exists := l.ingressInMemoryMap[podIdentifier]; !exists {
				...
				l.ingressInMemoryMap[podIdentifier], err = mapsv2.NewInMemoryBpfMap(&mapToUpdate)
				if err != nil {
					log().Errorf("got err for ingress in-mem map for %v, err %v", podIdentifier, err)
				} else {
					log().Infof("ingress map for %v loaded successfully", podIdentifier)
		...
		if egressProgInfo.Program.ProgFD != 0 {
			...
			if _, exists := l.egressInMemoryMap[podIdentifier]; !exists {
				...
				l.egressInMemoryMap[podIdentifier], err = mapsv2.NewInMemoryBpfMap(&mapToUpdate)
				if err != nil {
					log().Errorf("got err for egress in-mem map for %v, err %v", podIdentifier, err)
				} else {
					log().Infof("egress map for %v loaded successfully", podIdentifier)
...
func (l *bpfClient) updateEbpfMap(firewallRules []EbpfFirewallRules, inMemMap *mapsv2.InMemoryBpfMap) error {
	start := time.Now()
	duration := msSince(start)
